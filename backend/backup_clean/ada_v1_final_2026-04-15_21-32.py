...
tools = [
    ...
    {"function_declarations": [
        ...
        iterate_cad_tool, universal_control_tool
...
class AudioLoop:
    def __init__(self, video_mode=DEFAULT_MODE, on_audio_data=None, on_video_frame=None, on_cad_data=None, on_web_data=None, on_transcription=None, on_tool_confirmation=None, on_cad_status=None, on_cad_thought=None, on_project_update=None, on_device_update=None, on_error=None, input_device_index=None, input_device_name=None, output_device_index=None, kasa_agent=None):
        ...
        ALL_TOOLS = [
            ...
            "analyze_document", "edit_document", "universal_control",
            "get_system_stats", "calculate"
        ]
        ...
        # ---- MEMORY ENGINE (long-term memory) ----
        self.memory_engine = MemoryEngine(project_manager=self.project_manager)

    def _on_proactive_notification(self, notification):
        """Called by ProactiveEngine when it has something to notify."""
        if notification and self.on_transcription:
            text = notification.get("text", "")
            if text:
                asyncio.create_task(self.session.send(input=f"[PROACTIVO] {text}", end_of_turn=True))
                            ...
                            if fc.name in [
                                ...
                                if not confirmation_required:
                                    ...
                                    if self.on_tool_confirmation:
                                        ...
                                        # No confirmation callback — auto-deny for safety
                                        print(f"[ADA DEBUG] [DENY] No confirmation callback for '{fc.name}'. Auto-denying.")
                                        function_response = types.FunctionResponse(
                                            id=fc.id,
                                            name=fc.name,
                                            response={"result": "Tool denied: no confirmation callback configured."}
                                        )
                                        function_responses.append(function_response)
                                        continue
                    ...
                    if not is_reconnect:
                        ...
                        try:
                            ...
                            if memory_context:
                                ...
                                print(f"[ADA DEBUG] [MEMORY] Injected {len(memory_context)} chars of memory context")
                        except Exception as e:
