                            ...
                            if fc.name in [
                                ...
                                if fc.name == "generate_cad":
                                    ...
                                    try:
                                        ...
                                        response = "[Timeout esperando respuesta de C.I.S.C.O.]"
                                        for _ in range(60):  # 60 * 0.5s = 30s
                                            await asyncio.sleep(0.5)
                                            if respuesta_path.exists():
                                                resp_text = respuesta_path.read_text(encoding="utf-8").strip()
                                                if resp_text and resp_text != "# Consulta de ADA":
                                                    response = resp_text
                                                    break
                                        # Clear consultation file
                                        consult_path.write_text("# Consulta de ADA", encoding="utf-8")
                                        function_response = types.FunctionResponse(
                                            id=fc.id, name=fc.name, response={"result": response}
                                        )
                                    except Exception as e:
                                        function_response = types.FunctionResponse(
