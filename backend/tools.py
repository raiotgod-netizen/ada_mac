...
create_word_tool = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "paragraphs": {"type": "ARRAY", "description": "Array of text strings, each becomes a paragraph. Use empty string for blank line. Use lines starting with '# ' for headings, '## ' for subheadings.", "items": {"type": "STRING"}}
        },
...
create_powerpoint_tool = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "slides": {"type": "ARRAY", "description": "Array of slide descriptors. Each descriptor can be: a string (text-only slide), an object with 'title' and 'bullets' keys (title + bullet points), or an object with 'title' and 'content' keys (content paragraph).", "items": {"type": "OBJECT"}}
        },
...
find_file_tool = {
    "name": "find_file",
    "description": "Search for a file or folder by name (exact or partial match) across Desktop, Documents, Downloads, OneDrive, and known project directories. Returns all matching paths ranked by relevance (exact first). Use when the user mentions a file but you don't know the exact path, or when a previous path lookup failed.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "name": {"type": "STRING", "description": "File or folder name to search for (case-insensitive, supports partial match). E.g. 'inventario.xlsx', 'settings', 'proyecto'."}
        },
        "required": ["name"]
    }
}
...
resolve_path_tool = {
    "name": "resolve_path",
    "description": "Resolve a partial, relative, shorthand, or incomplete file path to a full absolute Windows path. Handles ~ (home), environment variables, relative paths, and bare filenames. Use when the user gives a partial path, a filename without a folder, or any path that might be ambiguous.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "path": {"type": "STRING", "description": "Partial, relative, or shorthand path to resolve. Examples: 'settings.json', '~/Documents/report.xlsx', '%USERPROFILE%\\Desktop\\notas.txt', 'ADA/ada_v1/backend/ada.py'."}
        },
        "required": ["path"]
    }
}
...
list_dir_tool = {
    "name": "list_dir",
    "description": "List the contents of a directory (files and subdirectories). If no path is provided, lists all known common locations (Desktop, Documents, Downloads, OneDrive, project roots). Use to explore folder structure or find what files are in a directory.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "path": {"type": "STRING", "description": "Optional. Directory path to list. If omitted, lists all known base directories."}
        },
        "required": []
    }
}
...
remember_tool = {
    "name": "remember",
    "description": "Guarda un hecho, preferencia o dato importante en la memoria persistente de largo plazo de ADA. Usa esto cuando el usuario diga 'recuerda que...', 'ten en cuenta que...', o proporcione información que deba persistir entre sesiones. El contenido se almacena con búsqueda semántica.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "content": {"type": "STRING", "description": "El hecho o dato a recordar. Debe ser texto natural y descriptivo. Ejemplo: 'A Diego le gusta el café con leche de almendras'."},
            "category": {"type": "STRING", "description": "Categoría: user_facts, preferences, project_context, general. Default: general", "enum": ["user_facts", "preferences", "project_context", "general"]},
            "tags": {"type": "ARRAY", "description": "Etiquetas para organizar. Ejemplo: ['nombre', 'usuario'].", "items": {"type": "STRING"}},
            "importance": {"type": "INTEGER", "description": "Importancia 1-5 (5=crítico). Default: 3."}
        },
        "required": ["content"]
    }
}
...
recall_tool = {
    "name": "recall",
    "description": "Busca en la memoria persistente de ADA. Usa cuando el usuario pregunte 'qué sabes de...', 'recuerdas que...'. Devuelve hechos relevantes encontrados.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "query": {"type": "STRING", "description": "Tema o pregunta a buscar. Ejemplo: 'preferencias de café de Diego'."},
            "top_k": {"type": "INTEGER", "description": "Máximo resultados (1-10). Default: 5."}
        },
        "required": ["query"]
    }
}
...
log_feedback_tool = {
    "name": "log_feedback",
    "description": "Registra feedback del usuario sobre una interaccion de ADA. Usa esto cuando el usuario diga 'buena respuesta', 'eso estuvo mal', o cualquier calificacion positiva o negativa sobre lo que ADA hizo o dijo.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "rating": {"type": "STRING", "description": "Calificacion: 'good' (respondio bien), 'bad' (respondio mal), 'ok' (aceptable).", "enum": ["good", "bad", "ok"]},
            "category": {"type": "STRING", "description": "Categoria: 'respuesta', 'tool', 'sugerencia', 'proactive', 'general'."},
            "note": {"type": "STRING", "description": "Comentario opcional del usuario. Max 300 caracteres."}
        },
        "required": ["rating"]
    }
}
...
execute_task_tool = {
    "name": "execute_task",
    "description": "Ejecuta una tarea compleja en background. Di 'haz esto' o 'prepara eso' y ADA lo descompone en pasos, lo ejecuta en background y te avisa cuando esté listo. Ideal para tareas de varios pasos como 'busca todos los archivos de un proyecto y crea un resumen', 'prepara la presentacion', etc.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "goal": {"type": "STRING", "description": "Objetivo de alto nivel en lenguaje natural. Ejemplo: 'Busca todos los archivos de configuracion del proyecto y crea un Excel con ellos'."}
        },
        "required": ["goal"]
    }
}
...
task_status_tool = {
    "name": "task_status",
    "description": "Consulta el estado de una tarea en background. Devuelve progreso, pasos completados y resultado final.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "task_id": {"type": "STRING", "description": "ID de la tarea devuelto por execute_task. Ejemplo: 'plan-a1b2c3d4'."}
        },
        "required": ["task_id"]
    }
}
...
list_tasks_tool = {
    "name": "list_tasks",
    "description": "Lista todas las tareas en background. Muestra ID, objetivo, estado y progreso. Filtra por estado opcionalmente.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "status": {"type": "STRING", "description": "Filtrar por estado: 'planning', 'running', 'completed', 'failed', 'cancelled'. O omitir para ver todas."}
        }
    }
}
...
cancel_task_tool = {
    "name": "cancel_task",
    "description": "Cancela una tarea en background que está en ejecución o planificación.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
